...
// including a big link on each page if you dont want to one will do just nicely. Feel
...
define('CACHE_DIR_NAME', '/datas/cache/');
...
if (!file_exists($docRoot.$image))
{
	...
	echo $L['sir_doesnotexist'].$docRoot.$image;
	exit;
}
...
{
	header('HTTP/1.1 400 Bad Request');
	echo $L['sir_notaccepted'].$docRoot.$image;
	exit;
}
...
if (!is_readable(CACHE_DIR))
{
	...
	echo $L['sir_cachenotreadable'];
	exit;
}
elseif (!is_writable(CACHE_DIR))
{
	header('HTTP/1.1 500 Internal Server Error');
	echo $L['sir_cachenotwritable'];
	exit;
}
