<!-- BEGIN: MAIN -->
			...
			<div id="left">
				...
				<p class="breadcrumb">{PHP.themelang.list.bread}: <a href="users.php">{PHP.L.Users}</a> {PHP.cfg.separator} <a href="users.php?m=details&amp;id={PHP.usr.id}&amp;u={PHP.usr.name}">{PHP.usr.name}</a> {PHP.cfg.separator} {PM_PAGETITLE} </p>
				...
				<!-- BEGIN: PM_ROW_EMPTY -->
				<p class="error">{PHP.themelang.list.none}</p>
				<!-- END: PM_ROW_EMPTY -->
				...
				<img src="themes/{PHP.theme}/img/icon-this.gif" alt="{PHP.L.Options}" />
				<input type="button" value="{PHP.themelang.pm.Selectall}" onClick="$('.checkbox').attr('checked', 'checked');" />
				<input type="button" value="{PHP.themelang.pm.Unselectall}" onClick="$('.checkbox').removeAttr('checked');" />
				<!-- ENDIF -->
