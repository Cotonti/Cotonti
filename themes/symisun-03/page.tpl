<!-- BEGIN: MAIN -->
			...
			<div id="left">
				...
				<p class="breadcrumb">{PHP.themelang.list.bread}: <a href="index.php">{PHP.L.Home}</a> {PHP.cfg.separator} {PAGE_CATPATH}</p>
				...
				<div class="multi">
					<strong>{PHP.themelang.page.Summary}: </strong>
				...
				<div class="seccat right small gray">
					...
					<span class="fleft">{PHP.themelang.index.by} {PAGE_OWNER}, {PAGE_BEGIN}
					<!-- IF {PHP.usr.isadmin} -->
					...
					<em>{PHP.themelang.page.update} {PAGE_DATE}</em>
					<!-- ELSE -->
					<em>{PAGE_DESC}</em>
					<!-- ENDIF -->
	...
	<div id="right">
		...
		<h3>{PHP.L.Download}</h3>
			...
			<div class="restrict padding15">
				{PHP.themelang.page.members}<br />
				<a href="users.php?m=auth">{PHP.L.Login} {PHP.themelang.forumspost.to} {PHP.L.Download}</a>
			</div>
			<!-- END: MEMBERSONLY -->
			...
			<div class="download padding15">
				...
				<span class="small">{PHP.themelang.page.downloaded}: {PAGE_FILE_COUNT}</span>
			</div>
			<!-- END: DOWNLOAD -->
		...
		<h3><a href="{PAGE_COMMENTS_RSS}">{PHP.themelang.list.rss}</a></h3>
