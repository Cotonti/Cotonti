<!-- BEGIN: MAIN -->
			...
			<div id="left" class="forums">
				...
				<p class="error">{PHP.skinlang.forumspost.privatetopic}</p>
				...
				<div class="post">
					...
					<span><!-- IF {FORUMS_POSTS_ROW_USERONLINE} -->{PHP.skinlang.forumspost.Onlinestatus1}<!-- ENDIF --></span></h4>
					<span class="postinfo fright"> <strong>{FORUMS_POSTS_ROW_CREATION}</strong> || <a href="forums.php?m=posts&amp;p={FORUMS_POSTS_ROW_ID}#p{FORUMS_POSTS_ROW_ID}">{PHP.skinlang.forumspost.permalink}</a> </span> <br class="clear" />
	...
	<div id="right">
		...
		<h3>{PHP.skinlang.forumspost.jump}<span class="whitee jump">{FORUMS_POSTS_JUMPBOX}</span></h3>
		<!-- IF {PHP.usr.isadmin} -->
