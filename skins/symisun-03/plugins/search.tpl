...
<div id="content">
	<div class="padding20 whitee">
		<div class="breadcrumb">{PHP.skinlang.list.bread}: <a href="index.php">{PHP.L.Home}</a>{PLUGIN_TITLE}</div>
		<h1>{PHP.L.Search}</h1>
		...
		<form id="search" action="{PLUGIN_SEARCH_ACTION}" method="post">
			...
			<div id="adminmenu">
				<ul>
					<li><a href="plug.php?e=search"

						   <!-- IF !{PHP.tab} -->
						   class="current"
						   <!-- ENDIF -->
						   >{PHP.L.plu_tabs_all}</a></li>
					<li><a href="plug.php?e=search&amp;tab=frm"

						   <!-- IF {PHP.tab} == "frm" -->
						   class="current"
						   <!-- ENDIF -->
						   >{PHP.L.plu_tabs_frm}</a></li>
					<li><a href="plug.php?e=search&amp;tab=pag"

						   <!-- IF {PHP.tab} == "pag" -->
						   class="current"
						   <!-- ENDIF -->
						   >{PHP.L.plu_tabs_pag}</a></li>
				</ul>
				<div class="clear"></div>
			</div>
			<div class="padding10" style="border:1px solid #ccc; background-color:#f9f9f9">
				<table class="cells">
					<tr>
						<td><div style="padding:15px 0 6px 15px;"> {PHP.L.plu_search_req}: {PLUGIN_SEARCH_TEXT} {PLUGIN_SEARCH_KEY}
