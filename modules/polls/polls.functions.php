...
{
	global $cfg, $db_polls, $db_polls_options;
	global $error_string;
	...
	elseif ((int)$id > 0)
	{
		...
		if ($row = sed_sql_fetcharray($sql))
		{
			...
			while ($row1 = sed_sql_fetcharray($sql1))
			{
				$counter++;
				$t->assign(array(
						"EDIT_POLL_OPTION_ID" => $row1['po_id'],
	...
	{
		$t->assign(array(
				"EDIT_POLL_CLOSE" => '<input name="poll_state" type="checkbox" value="1" $poll_state />',
				"EDIT_POLL_RESET" => '<input name="poll_reset" type="checkbox" value="1" />',
				"EDIT_POLL_DELETE" => '<input name="poll_delete" type="checkbox" value="1" />',
		));
		$t->parse($block.".EDIT");
	}
	...
	$t->assign(array(
			...
			"EDIT_POLL_ID" => $id,
			"EDIT_POLL_MULTIPLE" => '<input name="poll_multiple" type="checkbox" value="1" $poll_multiple />'
	));
...
function sed_poll_check()
{
	...
	if (!empty($poll_id))
	{
		...
		foreach ($poll_options as $key => $val)
		{
			$poll_options[$key]=trim(sed_import($val, 'D', 'TXT'));
		}
...
{
	global $sys, $db_polls, $db_polls_options;
	global $error_string;
	global $poll_id, $poll_text, $poll_multiple, $poll_state, $poll_options;
	...
	if (!empty($poll_id) && empty($error_string))
	{
		if ((int)$poll_id > 0)
		{
			$sql = sed_sql_query("UPDATE $db_polls SET  poll_state = '".(int)$poll_state."', poll_text = '".sed_sql_prep($poll_text)."',
									poll_multiple = '".(int)$poll_multiple."' WHERE poll_id = '$poll_id'");
			$newpoll_id = $poll_id;
		}
		else
		{
			$sql = sed_sql_query("INSERT INTO $db_polls (poll_type, poll_state, poll_creationdate, poll_text, poll_multiple, poll_code)
									VALUES ('".sed_sql_prep($type)."', ".(int)$poll_state.", ".(int)$sys['now_offset'].",
										'".sed_sql_prep($poll_text)."', '".(int)$poll_multiple."', '".(int)$code."')");
		...
		foreach($poll_options as $key => $val)
		{
			if (!empty($val))
			{
				$sql2 = sed_sql_query( ((int)$key > 0 && (int)$poll_id > 0) ?
						"UPDATE $db_polls_options SET po_text = '".sed_sql_prep($val)."' WHERE po_id = '".(int)$key."'"	:
						"INSERT into $db_polls_options (po_pollid, po_text, po_count) VALUES ('$newpoll_id', '".sed_sql_prep($val)."', '0')");
				$ids[] = $key;
			}
		}
