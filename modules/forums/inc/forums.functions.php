...
function forums_url_structure(&$args)
{
	...
	elseif (isset($args['m']) && $args['m'] == 'posts')
	{
		if (isset($args['q']))
		{
			$q = (int)$args['q'];
			$d = (int)$args['d'];
			$s = $db->query("SELECT fp_cat FROM $db_forum_posts WHERE fp_topicid=" .
				$q)->fetchColumn();

			$replacement .= str_replace('.','/',$structure['forums'][$s]['path']) .
				'/topic' . $q;
		...
		{
			$id = (int)$args['id'];
			$s = $db->query("SELECT fp_cat FROM $db_forum_posts WHERE fp_id=" .
				$id)->fetchColumn();
