...
if ($a == 'newpost' && !empty($s) && !empty($q))
{
	...
	if (!$cot_error && !empty($newmsg))
	{
		...
		else
		{
			...
			$sql = $db->query("UPDATE $db_forum_posts SET
				fp_updated='".$sys['now_offset']."', fp_updater='".$db->prep($rupdater)."',
				fp_text='".$newmsg."', fp_posterip='".$usr['ip']."' WHERE fp_id='".$row['fp_id']."' LIMIT 1");
...
while ($row = $sql->fetch())
{
	...
	{
		$row['fp_updatedby'] = sprintf($L['forums_updatedby'], htmlspecialchars($row['fp_updater']), 
			@date($cfg['dateformat'], $row['fp_updated'] + $usr['timezone'] * 3600), cot_build_timegap($row['fp_updated'], $sys['now_offset']));
	}
