...
$sql_users = Cot::$db->query(
    "SELECT DISTINCT u.*, o.*, $is_user_check FROM " . Cot::$db->online . ' AS o ' . $join_condition .' '. $where
...
if (
    (!Cot::$cfg['easypagenav'] && $durl > 0 && $maxuserssperpage > 0 && $durl % $maxuserssperpage > 0)
	|| ($d > 0 && $d >= $totallines)
) {
	cot_redirect(cot_url('whosonline'));
}
$pagenav = cot_pagenav('whosonline', ['d' => $durl], $d, $totallines, $maxuserssperpage);
