...
if (is_array($i18n_structure) && count($i18n_structure) > 0
	&& ($rs['pag']['sub'][0] == 'all' || count($i18n_search_cats) > 0))
{
	...
	if ($rs['pag']['sub'][0] == 'all' || count($i18n_search_cats) == 0)
	{
	...
	if ($rs['set']['limit'] > 0)
	{
		$where_and['date2'] = "ipage_date >= ".$rs['set']['from']." AND ipage_date <= ".$rs['set']['to'];
	}

	unset($where_or['title'], $where_or['desc'], $where_or['text']);

	$where_or['title'] = $rs['pag']['title'] ? "ipage_title LIKE '".$db->prep($sqlsearch)."'" : '';
	$where_or['desc'] = $rs['pag']['desc'] ? "ipage_desc LIKE '".$db->prep($sqlsearch)."'" : '';
	$where_or['text'] = $rs['pag']['text'] ? "ipage_text LIKE '".$db->prep($sqlsearch)."'" : '';
