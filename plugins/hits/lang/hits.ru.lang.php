<?php
/**
 * Russian Language File for Hits Plugin
 *
 * @package Hits
 * @version 0.9.0
 * @author Cotonti Translators Team
 * @copyright Copyright (c) Cotonti Team 2008-2011
 * @license BSD
 */

defined('COT_CODE') or die('Wrong URL.');

/**
 * Plugin Config
 */

$L['cfg_disableactivitystats'] = array('Не показывать статистику активности за последние 7 дней', '(отображается на главной странице админ-панели)');
$L['cfg_disabledbstats'] = array('Не показывать статистику базы данных', '(отображается на главной странице админ-панели)');
$L['cfg_hit_precision'] = array('Точность оптимизированного счётчика просмотров', '');

$L['indo_desc'] = 'Простая статистика просмотров. Рекомендуется только для небольших сайтов';

/**
 * Plugin Body
 */

$L['hits_maxhits'] = 'Максимальное количество хитов (%2$s) зафиксировано %1$s';

$L['hits_byyear'] = 'По годам';
$L['hits_bymonth'] = 'По месяцам';
$L['hits_byweek'] = 'По неделям';

$L['hits_hits'] = 'Посещаемость за последние 7 дней';
$L['hits_activity'] = 'Активность за последние 7 дней';

?>