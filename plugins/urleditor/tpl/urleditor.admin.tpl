...
<div class="block button-toolbar">
	<a href="{PHP|cot_url('admin', 'm=config&n=edit&o=plug&p=urleditor')}" class="button">{PHP.L.Configuration}</a>
</div>
...
<div class="block">
	<form name="add_url" id="add_url" action="{ADMIN_URLS_FORM_URL}" method="post" class="ajax marginbottom10">
	<table id="rules" class="cells">
		<tr id="rules_top" class="nodrop nodrag">
			<td class="coltop w-15">{PHP.L.adm_urls_area}</td>
			<td class="coltop w-35">{PHP.L.adm_urls_parameters}</td>
			<td class="coltop w-35">{PHP.L.adm_urls_format}</td>
			<td class="coltop w-15">{PHP.L.Delete}</td>
		</tr>
		<!-- BEGIN: ROW -->
		<tr id="rule_{ADMIN_URLS_ROW_I}">
			...
			<td class="centerall">
				<a href="#" onclick="$(this).parent().parent().remove();return false;">{PHP.L.Delete}</a>
			</td>
		</tr>
		<!-- END: ROW -->
		<tr id="rule_new">
