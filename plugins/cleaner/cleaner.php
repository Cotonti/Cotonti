...
if ($cfg['plugin']['cleaner']['userprune'] > 0)
{
	...
	$sqltmp1 = cot_db_query("SELECT user_id FROM $db_users WHERE user_maingrp='2' AND user_lastlog='0' AND user_regdate<$timeago");
	$sqltmp2 = cot_db_query("DELETE FROM $db_users WHERE user_maingrp='2' AND user_lastlog='0' AND user_regdate<$timeago");
	$deleted = cot_db_affectedrows();

	while ($row = cot_db_fetcharray($sqltmp1))
	{
		$sqltmp2 = cot_db_query("DELETE FROM $db_users WHERE user_id='".$row['user_id']."'");
		$sqltmp2 = cot_db_query("DELETE FROM $db_groups_users WHERE gru_userid='".$row['user_id']."'");
	}
...
if ($cfg['plugin']['cleaner']['logprune'] > 0)
{
	...
	$sqltmp = cot_db_query("DELETE FROM $db_logger WHERE log_date<$timeago");
	$deleted = cot_db_affectedrows();
	if ($deleted > 0)
	{
...
if ($cfg['plugin']['cleaner']['refprune'] > 0)
{
	...
	$sqltmp = cot_db_query("DELETE FROM $db_referers WHERE ref_date<$timeago");
...
if ($cfg['module']['pm'])
{
	...
	if ($cfg['plugin']['cleaner']['pmnotread'] > 0)
	{
		...
		$sqltmp = cot_db_query("DELETE FROM $db_pm WHERE pm_date<$timeago AND pm_tostate=0");
	...
	if ($cfg['plugin']['cleaner']['pmnotarchived'] > 0)
	{
		...
		$sqltmp = cot_db_query("DELETE FROM $db_pm WHERE pm_date<$timeago AND pm_tostate=1");
	...
	if ($cfg['plugin']['cleaner']['pmold'] > 0)
	{
		...
		$sqltmp = cot_db_query("DELETE FROM $db_pm WHERE pm_date<$timeago");
