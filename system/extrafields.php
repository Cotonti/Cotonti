...
function cot_build_extrafields($name, $extrafield, $data)
{
	...
	switch ($extrafield['field_type']) {
		...
		case 'textarea':
			...
			break;
		case 'select':
			$extrafield['field_variants'] = str_replace([' , ', ', ', ' ,'], ',', $extrafield['field_variants']);
                ...
                $parse='HTML', $description='', $params = '', $enabled = 1, $noalter = false, $customtype = '')
{
	...
	if ($step1) cot_log("Add extrafield '" . $name . "' in list table '" . $location . "'", 'adm', 'extrafield', 'add');
	if ($noalter) {
		return $step1;
	}
	switch ($type) {
...
function cot_extrafield_update($location, $oldname, $name, $type, $html='', $variants='', $default='', $required=0, $parse='HTML', $description='', $params = '', $enabled = 1, $customtype = '')
{
	...
	if ($step1) cot_log("Updated extrafield '" . $oldname . "' in list table '" . $location . "'", 'adm', 'extrafield', 'update');
	...
	$step2 = $db->query($sql);
	if ($step2) cot_log("Updated extrafield '" . $name . "' in DB table '" . $location . "'", 'adm', 'extrafield', 'update');
...
function cot_extrafield_remove($location, $name)
{
	...
	if ($step1) cot_log("Deleted extrafield '" . $name . "' from list table '" . $location . "'", 'adm', 'extrafield', 'delete');
...
function cot_extrafields_register_table($table_name)
{
	if (!isset(Cot::$extrafields[Cot::$db->{$table_name}])) {
		Cot::$extrafields[Cot::$db->{$table_name}] = [];
	}
}
