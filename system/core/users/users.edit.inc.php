...
if ($a=='update')
{
	...
	if ($ruserdelete)
	{
		if ($sys['user_istopadmin'] && !$sys['edited_istopadmin'])
		{
			...
			if ($row = sed_sql_fetchassoc($sql))
			{
				...
				header("Location: " . SED_ABSOLUTE_URL . "message.php?msg=109&rc=200&id=$id");
				exit;
			}
		}
		else
	...
	if (empty($error_string))
	{
		...
		header("Location: " . SED_ABSOLUTE_URL . "users.php?m=edit&id=".$id);
		exit;
	}
}
...
$t->assign(array(
	...
	"USERS_EDIT_TEXT" => "<textarea name=\"rusertext\" rows=\"4\" cols=\"56\">".sed_cc($urr['user_text'])."</textarea>",
	"USERS_EDIT_TEXTBOXER" => "<textarea name=\"rusertext\" rows=\"4\" cols=\"56\">".sed_cc($urr['user_text'])."</textarea>",
