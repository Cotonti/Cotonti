...
<div class="block">
	<form action="{ADMIN_EXTRAFIELDS_URL_FORM_EDIT}" method="post">
		<div class="wrapper">
			<table class="cells">
				<tr>
					<td class="coltop"></td>
					<td class="coltop w-40">{PHP.L.extf_Name}</td>
					<td class="coltop">{PHP.L.extf_Type}</td>
					<td class="coltop w-40">{PHP.L.adm_extrafield_params}</td>
	...
	<p class="paging">
		...
		<span>{PHP.L.Total}: {ADMIN_EXTRAFIELDS_TOTALITEMS}, {PHP.L.Onpage}: {ADMIN_EXTRAFIELDS_ON_PAGE_ITEMS}</span>
	</p>
	<!-- ENDIF -->
</div>
